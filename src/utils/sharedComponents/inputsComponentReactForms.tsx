...
export const InputOfUrlImagesForm = (props: InputFormProps): React.JSX.Element => {
    ...
    return <div className="mb-5">
        <div className="w-24 h-24 mb-2 bg-white border border-slate-400 rounded-lg shadow-xs dark:bg-gray-800 dark:border-gray-700 mx-auto sm:mx-0">
            {props.currentValue &&
...
export const ValidateButton = (props: ButtonProps): React.JSX.Element => {
    return <button name="submitButton" onClick={props.functionToDo} type="submit" className={"text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-hidden focus:ring-blue-300 font-medium rounded-lg text-lg w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 " + (props.styleOverride ?? "")}>{props.title}</button>
}


export const CancelButton = (props: ButtonProps): React.JSX.Element => {
    return <button type="button" name="cancelButton" onClick={(e) => { e.preventDefault(); props.functionToDo() }} className={"text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-hidden focus:ring-red-300 font-medium rounded-lg text-lg w-full sm:w-auto px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-blue-800 " + (props.styleOverride ?? "")}>{props.title}</button>
}

export const DeleteButton = (props: ButtonProps): React.JSX.Element => {
    return <button type="button" name="deleteButton" onClick={(e) => { e.preventDefault(); props.functionToDo() }} className={"text-white bg-red-800 hover:bg-red-900 focus:ring-4 focus:outline-hidden focus:ring-red-300 font-medium rounded-lg text-xs w-full sm:w-auto px-5 py-2.5 text-center dark:bg-red-800 dark:hover:bg-red-900 dark:focus:ring-blue-800 " + (props.styleOverride ?? "")}>{props.title}</button>
}

export const FixedButton = (props: ButtonProps): React.JSX.Element => {
    return <button id="fixedButton" onClick={props.functionToDo} className={"fixed bottom-4 right-2 text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-hidden focus:ring-blue-300 font-medium rounded-lg text-lg px-2.5 py-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 " + (props.styleOverride ?? "")}>
    ...
    return (
        <div className={"flex gap-[6px]  " + (props.styleOverride ?? "")}>
            ...
            <div className="flex gap-[2px] max-w-[200px] h-[35px] overflow-auto  items-center px-[2px]">
                <input className="max-w-[50px] bg-white rounded-md text-center dark:text-white dark:bg-gray-400 h-[25px]" value={props.currentPage !== 0 ? props.currentPage : ''} onChange={(event) => { handleInputChange(event, props.handlerCurrentPage) }} />
...
export const MinusPlusInput = (props: MinusPlusInputInterface): React.JSX.Element => {
    ...
    return <>
        ...
        <input id={props.id + "input"} className="mx-2 border border-gray-200 text-center w-16 h-10  dark:bg-white" value={emptyValue ? '' : props.value} onChange={(event) => { handleInputChange(event, props.modifyValue) }} />
